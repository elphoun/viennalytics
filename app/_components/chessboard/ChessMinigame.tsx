...
const ChessMinigame: React.FC = () => {
  const initialFen = "8/7N/8/3k4/8/2K5/5B2/8 w - - 0 1";
  const timeLimit = 30;
  ...
  const makeMove = useCallback(
    ...
    [game, isGameActive, isGameStarted, timeLeft],
  );
  ...
  return (
    <div className="flex flex-col items-center gap-4 p-4 bg-gray-900 rounded-lg">
      <div className="flex items-center gap-4 mb-2">
        <div
          className={`text-lg font-bold ${timeLeft <= 10 ? "text-red-400" : "text-white"}`}
        >
      ...
      {gameResult && (
        <div
          className={`text-lg font-bold ${gameResult.includes("Checkmate") ? "text-green-400" : "text-red-400"}`}
        >
          {gameResult}
        </div>
      )}
      ...
      <div className="text-sm text-gray-400 text-center max-w-md">
        ...
        <p>WHITE STARTS</p>
        <p>Position: 8/7N/8/3k4/8/2K5/5B2/8 w - - 0 1</p>
      </div>
    </div>
  );
};
