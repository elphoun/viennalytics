...
const EvalBar = memo(({ evaluation, status = 'normal' }: EvalBarProps) => {
    ...
    if (status === 'checkmate') {
        ...
        displayText = centipawns > 0 ? `+${centipawns.toFixed(1)}` : centipawns.toFixed(1);
    }
    ...
    return (
        <div className="relative h-[400px] w-6 flex-shrink-0">
            <div className={cn(
                "h-full w-full flex flex-col overflow-hidden rounded-sm bg-neutral-700",
                ...
                <div
                    className={cn(
                        ...
                        status === 'checkmate' && evaluation < 50 ? "bg-green-600" : "bg-neutral-900"
                    )}
                ...
                <div
                    className={cn(
                        "w-full transition-all duration-300 ease-in-out",
                        status === 'checkmate' && evaluation > 50 ? "bg-green-600" : "bg-neutral-100"
                    )}
            ...
            <div className="absolute inset-0 flex items-center justify-center">
                <span
                    className={cn(
                        "font-bold text-xs pointer-events-none px-1 py-0.5 rounded bg-gray-500",
                        textColorClass,
                        isGameOver && "bg-yellow-400 text-black",
                        isCheck && "bg-red-400 text-white"
                    )}
                    style={{
                        textShadow: !isGameOver && !isCheck ? "1px 1px 2px rgba(0, 0, 0, 0.5)" : "none",
