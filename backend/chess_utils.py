...
# Set up logging
logger = setup_logging(level="INFO", module_name="chess_utils")
...
class PerformanceMonitor:
    ...
    def checkpoint(self, name):
        ...
        logger.info(f"{name}: {elapsed:.2f}s (Total: {total_elapsed:.2f}s)")
    ...
    def print_stats(self, stage=""):
        ...
        if self.process:
            ...
            logger.info(f"[{stage}] Time: {stats['elapsed_seconds']:.1f}s, "
                       f"Memory: {stats['memory_mb']:.1f}MB, "
                       f"CPU: {stats['cpu_percent']:.1f}%")
        else:
            total_elapsed = time.time() - self.start_time
            logger.info(f"{stage}: Total time: {total_elapsed:.2f}s")
