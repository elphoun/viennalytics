...
def load_eco_data():
    ...
    for path_pattern in possible_paths:
        ...
        if eco_files:
            print(f"Found ECO files at: {path_pattern}")
            break
    
    if not eco_files:
        print("Warning: No ECO files found in any of the expected locations:")
        for path in possible_paths:
            print(f"  - {path}")
        print("Opening classification will be limited. Create data/eco/ directory with ECO JSON files for full functionality.")
        return {}  # Return empty dict instead of raising error

    print(f"Loading {len(eco_files)} ECO files...")
    for eco_file in eco_files:
        try:
            with open(eco_file) as f:
                eco_data.update(json.load(f))
        except Exception as e:
            print(f"Error loading {eco_file}: {e}")
...
print(f"Loaded {len(eco_openings)} openings with {len(move_tree)} move tree entries")