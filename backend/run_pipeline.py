...
def run_step(step_number, script_name, description, logger):
    """
    ...
    """
    logger.info(f"Starting step {step_number}: {description}")
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            logger.error(
                f"Step {step_number} failed with return code {result.returncode}"
            )
            return False

    except Exception as e:
        logger.error(f"Step {step_number} failed with error: {e}")
        return False
...
def main():
    ...
    # Set up logging
    logger = setup_logging(level="INFO", module_name="pipeline_runner")
    ...
    if args.list:
        logger.info("Chess Data Analysis Pipeline Steps:")
        for step_num, script, desc in pipeline_steps:
            logger.info(f"Step {step_num}: {desc}")
            logger.info(f"  Script: {script}")
        return

    logger.info("Starting Chess Data Analysis Pipeline")
    logger.info("This pipeline will process chess data through multiple stages")
    logger.info("Each step depends on the output of previous steps")
    ...
    if args.step:
        ...
        if not steps_to_run:
            logger.error(f"Invalid step number {args.step}. Valid steps are 1-5.")
            return
    elif args.from_step:
        # Run from specific step onwards
        steps_to_run = [step for step in pipeline_steps if step[0] >= args.from_step]
        if not steps_to_run:
            logger.error(f"Invalid step number {args.from_step}. Valid steps are 1-5.")
            return
    else:
        # Run all steps
        steps_to_run = pipeline_steps

    log_data_processing(logger, "Steps to run", len(steps_to_run))

    # Create output directory
    output_dir = Path("data/generated_data")
    output_dir.mkdir(parents=True, exist_ok=True)
    log_file_operation(logger, "Created output directory", str(output_dir))
    ...
    for step_num, script, description in steps_to_run:
        success = run_step(step_num, script, description, logger)
    ...
    if successful_steps == total_steps:
        logger.info(f"All {total_steps} steps executed without errors")
        logger.info(f"Generated files are in: {output_dir}")

        # List generated files
        if output_dir.exists():
            generated_files = list(output_dir.glob("*.json")) + list(
                output_dir.glob("*.png")
            )
            if generated_files:
                log_data_processing(logger, "Generated files", len(generated_files))
                for file in sorted(generated_files):
                    logger.info(f"  - {file.name}")
    else:
        logger.error(
            f"Pipeline incomplete: {successful_steps}/{total_steps} steps completed"
        )
