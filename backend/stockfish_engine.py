...
import chess
from config import MAX_WORKERS, STOCKFISH_CACHE_SIZE, STOCKFISH_DEPTH, STOCKFISH_PATH
from logging_config import log_error_with_context, setup_logging
from stockfish import Stockfish

# --- Constants ---
CACHE_SIZE = STOCKFISH_CACHE_SIZE  # Use the value from config

# Set up logging
logger = setup_logging(level="INFO", module_name="stockfish_engine")
...
if not STOCKFISH_AVAILABLE:
    logger.warning(f"Stockfish binary not found at {STOCKFISH_PATH}")
    logger.warning(
        "Opening evaluations will be disabled. Install Stockfish to enable evaluations."
    )
    engine_pool = None
else:
    logger.info(f"Stockfish found at {STOCKFISH_PATH}, initializing engine pool")
    ...
    try:
        ...
        for move_info in top_moves:
            ...
            try:
                ...
                result_moves.append(
                    {"move": move_san, "fen": resulting_fen, "eval": eval_value}
                )
            except Exception:
                # Skip invalid moves
                continue
...
def batch_get_stockfish_evals(fen_list, max_workers=None):
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        future_to_idx = {
            executor.submit(evaluate_fen, (idx, fen)): idx
        ...
        for future in as_completed(future_to_idx):
            try:
                ...
                log_error_with_context(logger, e, f"evaluating FEN at index {idx}")
