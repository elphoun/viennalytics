...
def main():
    ...
    logger = setup_logging(level="INFO", module_name="opening_stats")
    ...
    try:
        ...
        log_data_processing(logger, "Loaded games from", len(df))
    except Exception as e:
        log_error_with_context(logger, e, f"loading input file {INPUT_FILE}")
        return
    ...
    try:
        ...
        log_file_operation(logger, "Saved opening stats to", OUTPUT_FILE)
    except Exception as e:
        log_error_with_context(logger, e, f"writing output file {OUTPUT_FILE}")
        return
