...
# Set up logging
logger = setup_logging(level="INFO", module_name="visualization_processors")
...
def save_visualization_data(data: Any, filename: str, description: str = "") -> None:
    ...
    try:
        ...
        log_file_operation(logger, "Generated", str(output_file))
        logger.info(f"  Description: {description}")
        log_data_processing(logger, "Records", data_size)
...
def main():
    ...
    """
    logger.info("Chess Visualization Processors - Example Usage")
    ...
    log_data_processing(logger, "ELO Distribution records", len(elo_dist))

    win_rates = process_win_rates(sample_opening_stats)
    log_data_processing(logger, "Win Rates openings", len(win_rates))

    eval_data = process_opening_eval_distribution(sample_opening_stats)
    log_data_processing(logger, "Evaluation Data evaluations", sum(len(v) for v in eval_data.values()))

    histogram_data = create_elo_histogram_data(elo_dist)
    log_data_processing(logger, "Histogram Data bins", len(histogram_data['bins']))

    heatmap_data = process_move_popularity_heatmap(sample_opening_stats)
    logger.info(f"Heatmap Data: {heatmap_data['size']}x{heatmap_data['size']} matrix")

    summary = generate_dataset_summary(sample_opening_stats)
    log_data_processing(logger, "Dataset Summary total games", summary['datasetOverview']['totalGames'])

    logger.info("All processor functions working correctly!")
