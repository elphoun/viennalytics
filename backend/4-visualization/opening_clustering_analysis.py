...
def load_win_rates_data(logger):
    ...
    logger.error("Please run generate_visualization_data.py first")
    return None
...
def create_cluster_visualization(results, logger):
    """
    ...
    log_file_operation(logger, "Enhanced visualization saved", str(plot_file))
    return plot_file
...
def print_cluster_summary(results, logger):
    ...
    """
    logger.info("="*80)
    logger.info("ENHANCED CHESS OPENING CLUSTERING ANALYSIS SUMMARY")
    logger.info("="*80)
    ...
    logger.info(f"   • {white_dominant} cluster(s) favor White, {black_dominant} favor Black, {balanced} are balanced")
    ...
    logger.info(f"   • Most decisive style: {most_decisive['label']} ({most_decisive['characteristics']['decisiveness']:.1f}% decisive)")
    logger.info(f"   • Most drawish style: {most_drawish['label']} ({most_drawish['characteristics']['draw_rate']:.1f}% draws)")
...
def main():
    ...
    create_cluster_visualization(results, logger)
    ...
    logger.info("  - data/generated_data/opening_cluster_visualization.png")
