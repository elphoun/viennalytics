...
const InfoDisplayGrid = ({ title, smallTitle, help, className, children }: InfoDisplayGridProps) => {
  ...
  return (
    <div
      className={cn(
        "relative group bg-white/10 backdrop-blur-sm border-2 border-amber-400/50 w-full flex flex-col items-center gap-1 justify-center h-fit rounded-lg p-1",
        className
      )}
    >
      ...
      <div className="w-full h-full flex items-center justify-center text-center min-h-0 overflow-hidden">
          {children}
      </div>
    </div>
  );
