...
import ViennalyticsLogo from '../../icons/ViennalyticsLogo';
import { cn } from '../../utils';

// ─ Constants ────────────────────────────────────────────────────────────────────────────────────
const CONTENT = {
  logo: { alt: 'Viennalytics', },
  navigation: {
    report: 'Report',
    opening: 'Opening',
  },
...
const Header = memo(() => (
  <header className={cn("flex flex-row items-center justify-between w-full px-5 py-1 transition-all duration-700 shadow-xl backdrop-blur-sm shadow-white/5 rounded-2xl md:px-8 bg-white/10")}>
    <Link
      to="/"
    ...
    >
      <div className={cn("relative flex flex-row items-center gap-3 ")}>
        <ViennalyticsLogo
          className={cn("transition-all duration-500 ease-out group-hover:drop-shadow-[0_0_20px_rgba(251,191,36,0.3)] group-hover:brightness-110 filter")}
          size={30}
        />
